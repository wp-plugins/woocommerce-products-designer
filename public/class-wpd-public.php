...
//                }
	...
	public function enqueue_scripts() {
            ...
            wp_localize_script( $this->wpd, 'ajax_object',
        ...
        //        var_dump($customization_list["output"]);
                ...
                if ( get_option('permalink_structure') ){
                    ...
                    $edit_item_url=  WPD_Product::get_url($base_product).'&edit='.$cart_item_key ;
            }
        ...
        public function wpd_add_rewrite_rules($aRules) {
            GLOBAL $wpc_options_settings;
            global $wp_query;
            $options=$wpc_options_settings['wpc-general-options'];
            $wpc_page_id = $options['wpc_page_id'];
            ...
            if (is_object($wpc_page)) {
                ...
                //$aNewRules = array();
                //$aNewRules = array($slug.'/([^/]+)/' => 'index.php?pagename='.$slug.'&product_id=$matches[1]');
                $aNewRules = array(
                    $slug.'/design'.'/([^/]+)/?$' => 'index.php?pagename='.$slug.'&product_id=$matches[1]',
                    $slug.'/design'.'/([^/]+)/([^/]+)/?$' => 'index.php?pagename='.$slug.'&product_id=$matches[1]&tpl=$matches[2]',
                    $slug.'/edit'.'/([^/]+)/([^/]+)/?$' => 'index.php?pagename='.$slug.'&product_id=$matches[1]&edit=$matches[2]',
                    $slug.'/ordered-design'.'/([^/]+)/([^/]+)/?$' => 'index.php?pagename='.$slug.'&product_id=$matches[1]&oid=$matches[2]',
                    $slug.'/saved-design'.'/([^/]+)/([^/]+)/?$' => 'index.php?pagename='.$slug.'&product_id=$matches[1]&design_index=$matches[2]',

                );            
                $aRules = $aRules + $aNewRules;
            }
            //var_dump($aRules);
