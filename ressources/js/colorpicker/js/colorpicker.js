	...
	var ColorPicker = function () {
		var
			...
			fillRGBFields = function  (hsb, cal) {
				...
				$(cal).data('wpc-colorpicker').fields
					.eq(1).val(rgb.r).end()
					.eq(2).val(rgb.g).end()
					.eq(3).val(rgb.b).end();
			},
			...
			setSelector = function (hsb, cal) {
				$(cal).data('wpc-colorpicker').selector.css('backgroundColor', '#' + HSBToHex({h: hsb.h, s: 100, b: 100}));
			...
			moveHue = function (ev) {
				change.apply(
					ev.data.cal.data('wpc-colorpicker')
						.fields
						.eq(4)
				...
				return this.each(function () {
					if (!$(this).data('wpc-colorpickerId')) {
						...
						cal
							...
							.find('>div.wpc-colorpicker_current_color').bind('click', restoreOriginal);
						options.selector = cal.find('div.wpc-colorpicker_color').bind('mousedown', downSelector);
						options.selectorIndic = options.selector.find('div div');
						options.el = this;
						options.hue = cal.find('div.wpc-colorpicker_hue div');
						cal.find('div.wpc-colorpicker_hue').bind('mousedown', downHue);
						...
						cal.find('div.wpc-colorpicker_submit')
